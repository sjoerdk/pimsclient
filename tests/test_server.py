...
@pytest.mark.parametrize("mock_response", [RequestsMockResponseExamples.KEYFILE_DOES_NOT_EXIST,
...
def test_swagger_error_responses(mock_pims_session, mock_response):
    ...
    with pytest.raises(PIMSServerException):
        mock_pims_session.get("a_url")  # actual url does not matter as mock response is returned by requests
